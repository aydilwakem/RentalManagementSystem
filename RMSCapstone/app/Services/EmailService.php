...
use App\Mail\NewDayTourReservationMail;
...
class EmailService
{
    ...
    public function sendDayTourReservationEmails(array $reservationData, string $pdfContent): void
    {
        try {
            //email for guest
            Mail::to($reservationData['email'])->send(new DayTouReservationSubmittedMail($reservationData, $pdfContent));
            Log::info('DayTourSubmitted sent to: ' . $reservationData['email']);

            //emails for admins
            Mail::to('username@try.com')->send(
                new NewDayTourReservationMail($reservationData)
            );
            Log::info('NewDayTourMail sent to: username@try.com');

            Mail::to('rmscapstone26@gmail.com')->send(
                new NewDayTourReservationMail($reservationData)
            );
            Log::info('NewDayTourMail also sent to: rmscapstone26@gmail.com');
        } catch (\Exception $e) {
            Log::error('Reservation email send failed: ' . $e->getMessage());
            throw $e;
        }
