        ...
        <div class="mx-auto max-w-2xl lg:py-2">
            ...
            <form wire:submit.prevent="">
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    ...
                    <!-- Rooms -->
                    <div>
                        <label for="property_id" class="block mb-2 text-sm font-medium text-gray-900">Room</label>
                        <select wire:model="property_id" id="property_id"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5">
                            <option value="">Select Room</option>
                            @foreach ($rooms as $room)
                                <option value="{{ $room->id }}">{{ $room->name_number }} - {{$room->amount}}</option>
                            @endforeach
                        </select>
                        @error('property_id')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                    ...
                    <p><strong>Total Pax:</strong> {{ $pax }}</p>
