<div class="min-h-[550px] container mx-auto p-6 max-w-full">
    ...
    @else
            ...
            <div class="flex items-center justify-between">
                ...
                @can('daytour-create')
                    <div class="flex justify-between items-center mb-4">
                        <x-button icon="fas fa-plus" onclick="window.location.href='{{ route('admin.create-day-tour') }}'">
                            New Day Tour
                        </x-button>
                    </div>
                @endcan
                <!-- Soft Deletes -->
                @can('daytour-soft-delete')
                    <x-button
                        class=" mb-4 !bg-gray-600 hover:!bg-gray-700 focus:ring focus:!ring-gray-600 focus:!ring-offset-2"
                        icon="fas fa-trash" href="{{ route('admin.deleted-day-tours') }}">
                        Deleted Tours
                    </x-button>
                @endcan
            </div>
                    ...
                    <tbody wire:loading.remove wire:target="search, statusFilter" class="dark:bg-gray-700">
                                ...
                                <td class="px-4 py-3 flex   space-x-2 mt-1">
                                    ...
                                    @can('daytour-delete')
                                        <i class="fas fa-trash-alt text-gray-700 hover:text-red-600 cursor-pointer dark:text-gray-200 hover:dark:text-red-500"
                                            wire:click="confirmDelete({{ $tour->id }})" wire:loading.attr="disabled">
                                        </i>
                                    @endcan
                                </td>
                            </tr>
                        @empty
                            <tr>
                                ...
                                <td colspan="8" class="text-center py-10 text-gray-500 dark:text-white">
                                    No day tours found.
                                </td>
                            </tr>
                        @endforelse
        ...
        <x-dialog-modal wire:model.live="confirmBulkDelete" type="danger">
            ...
            <x-slot name="content">
                {{ __('Are you sure you want to delete these day tours?') }}
            </x-slot>
        ...
        <x-dialog-modal wire:model="cannotDeleteItem" type="ghost">
            ...
            <x-slot name="content">
                {{ __('These day tours have associated rates and cannot be deleted.') }}
            </x-slot>
