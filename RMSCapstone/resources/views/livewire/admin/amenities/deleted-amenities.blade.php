        ...
        <div>
                        ...
                        @foreach ($deletedAmenities as $amenity)
                            <tr class="border-b">
                                <td class="px-4 py-3 font-medium text-gray-900 text-left">
                                    {{ $fakeIDs[$amenity->id] ?? 'AMY-???' }}</td>
                                <td class="px-4 py-3 text-left">{{ $amenity->name }}</td>
                                <td class="px-4 py-3 space-x-2 text-center">
                                    <x-button wire:click="restoreAmenity({{ $amenity->id }})" class="mb-2">
                                        Restore
                                    </x-button>
                                    <!-- Delete Forever Button -->
                                    <x-danger-button wire:click="confirmDeleteForever({{ $amenity->id }})">
                                        Delete Forever
                                    </x-danger-button>
                                </td>
                            </tr>
            ...
            <x-dialog-modal wire:model.live="confirmItemDelete">
                ...
                <x-slot name="footer">
                    ...
                    <x-danger-button class="ms-3" wire:click="deleteAmenityForever({{ $amenity->id }})"
