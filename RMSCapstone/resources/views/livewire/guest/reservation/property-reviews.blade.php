<div x-data="{ open: @entangle('showReviewsModal') }">
    ...
    <button x-on:click="$wire.showReviews()"
        ...
        <div class="flex text-yellow-500">
            @for ($i = 1; $i <= 5; $i++) @if ($i <=floor($averageRating)) <i class="fas fa-star"></i>
                ...
                <div class="w-full lg:w-1/3 pr-6 border-r border-gray-200 sticky top-0">
                    ...
                    <div class="space-y-3 mb-8">
                        ...
                        <div class="flex items-center">
                            ...
                            <span class="text-sm text-gray-500 ml-3 w-10 text-right">{{ $totalReviews > 999 ?
                                number_format($ratingBreakdown[$i] / 1000, 1) . 'k' : $ratingBreakdown[$i] }}</span>
                        </div>
                        @endfor
                    </div>
                ...
                <div class="w-full lg:w-2/3 lg:pl-6 space-y-6 pb-12">
                    ...
                    <div class="flex items-center space-x-2 text-sm">
                        ...
                        <select wire:model.live="reviewSort"
                            class="border-none bg-gray-50 text-gray-700 font-medium text-sm rounded-lg focus:ring-green-500 focus:border-green-500 py-1 pl-3 pr-8 cursor-pointer hover:bg-gray-100 transition-colors">
                    ...
                    <div
                        ...
                        <div class="flex items-start justify-between mb-3">
                            ...
                            <div class="flex space-x-4 text-gray-400">
                                <div wire:click="getLikesCount({{ $review['id'] }})"
                                    class="flex items-center space-x-1 hover:text-blue-500 cursor-pointer transition-colors">
                                    <span class="text-sm">{{ $review['feedback_likes'] }}</span>
                                    <i class="far fa-thumbs-up
                                    @if(in_array($review['id'], $liked)) text-blue-600 
                                    @else text-gray-500 
                                    @endif">
                                    </i>
                                </div>
                                ...
                                <!-- Thumbs Down -->
                                {{-- <div wire:click="getDislikesCount({{ $review['id'] }})"
                                    class="flex items-center space-x-1 hover:text-red-500 cursor-pointer transition-colors">
                                    <span class="text-sm">{{ $review['feedback_dislikes'] }}</span>
                                    <i class="far fa-thumbs-up
                                    @if(in_array($review['id'], $disliked)) text-red-500 
                                    @else
                                        text-gray-500 
                                    @endif">
                                    </i>
                        ...
                        <div class="flex items-center space-x-2 mb-3">
                            <span class="px-2 py-0.5 bg-yellow-500 text-gray-900 text-xs font-semibold rounded-full">
                                {{ $review['overall_rating'] }}
                            </span>
                            <div class="flex text-yellow-500 text-sm">
                                @for ($i = 1; $i <= 5; $i++) @if ($i <=$review['overall_rating']) <i
                                    class="fas fa-star"></i>
                                    @else
                                    <i class="far fa-star text-gray-300"></i>
                                    @endif
                            ...
                            {{-- <span class="text-sm text-gray-700 font-semibold">{{ $review['overall_rating'] }} /
                                5</span> --}}
                        </div>
                        ...
                        <div class="flex flex-wrap gap-x-6 gap-y-2 text-xs font-medium">
                            ...
                            <div class="flex items-center space-x-1 text-gray-600">
                                ...
                                <div class="flex">
                                    @for ($i = 1; $i <= 5; $i++) @if ($i <=$rating['value']) <i
                                        class="fas fa-star text-yellow-400 text-[9px]"></i>
                                        @else
