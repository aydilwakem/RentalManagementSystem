                ...
                <div class="bg-gray-50 rounded-lg p-6 mb-6 border border-gray-200">
                    <div class="flex flex-col md:flex-row items-center justify-between gap-6">
                        <div class="text-center flex-1">
                            ...
                            <div class="text-sm text-gray-600 mt-2">{{ $totalReviews }} review{{ $totalReviews > 1 ? 's' : '' }}</div>
                        </div>
                ...
                <div class="space-y-6">
                    @forelse ($propertyReviews as $review)
                        <div class="border-b border-gray-200 pb-6 last:border-b-0">
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center space-x-1 mt-1">
                                        @for ($i = 1; $i <= 5; $i++)
                                            ...
                                            @endif
                                        @endfor
                                        <span class="text-sm text-gray-600 ml-1">{{ $review['overall_rating'] }}/5</span>
                                    </div>
                                </div>
                            ...
                            <p class="text-gray-700 mb-4 bg-gray-50 p-4 rounded-lg text-justify">{{ $review['comments'] }}</p>
                            @endif
                                    ...
                                    @if ($rating['type'] !== 'Overall Experience')
                                        <div class="flex items-center justify-between py-1">
                                            ...
                                            <div class="flex items-center space-x-1">
                                                <span class="text-yellow-500 font-medium">{{ $rating['value'] }}/5</span>
                                                <div class="flex">
                                                    @for ($i = 1; $i <= 5; $i++)
                                                        @if ($i <= $rating['value'])
                                                            <i class="fas fa-star text-yellow-400 text-xs"></i>
