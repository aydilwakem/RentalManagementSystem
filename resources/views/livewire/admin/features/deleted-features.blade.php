<div class="min-h-[550px] container mx-auto p-6 ">
    ...
    @if ($deletedFeatures->isEmpty())
        ...
        <div class="text-center py-10">
            <p class="text-gray-500 text-lg font-semibold">No deleted features yet.</p>
        </div>
    @else
        ...
        <div>
                ...
                <table class="w-full text-left">
                    <thead class="text-sm text-gray-700 bg-gray-200">
                        <tr>
                            ...
                            <!-- Room Category Name -->
                            <th scope="col" class="px-4 py-3 text-left">Feature Name</th>
                    ...
                    <tbody class="text-center">
                        @foreach ($deletedFeatures as $feature)
                            <tr class="border-b">
                                <td class="px-4 py-3 font-medium text-gray-900 text-left">
                                    {{ $fakeIDs[$feature->id] ?? 'FTR-???' }}</td>
                                <td class="px-4 py-3 text-left">{{ $feature->name }}</td>
                                <td class="px-4 py-3 space-x-2 text-center">
                                    <x-button wire:click="restoreFeature({{ $feature->id }})" class="mb-2">
                                        Restore
                                    </x-button>
                                    <!-- Delete Forever Button -->
                                    <x-danger-button wire:click="confirmDeleteForever({{ $feature->id }})">
                                        Delete Forever
                                    </x-danger-button>
                                </td>
                            </tr>
            ...
            <x-dialog-modal wire:model.live="confirmItemDelete" type="danger">
                ...
                <x-slot name="footer">
                    ...
                    <x-danger-button class="ms-3" wire:click="deleteFeatureForever({{ $feature->id }})"
