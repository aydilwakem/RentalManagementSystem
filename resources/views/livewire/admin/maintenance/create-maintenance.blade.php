<div>
    ...
    <x-slot name="header">
        ...
        <x-breadcrumbs :items="[
            ...
            ['label' => 'Create Maintenance Report', 'url' => route('admin.create-maintenance')],
        ]" />
    </x-slot>
            ...
            <form wire:submit.prevent="">
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" wire:model.live="resolved_at" id="resolved_at"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-600 focus:border-green-600 block w-full p-2.5
                            dark:bg-gray-600 dark:border-gray-500 dark:text-white dark:placeholder-gray-400">
                        @error('resolved_at')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                    ...
                    <div x-data="{ showPlanned: @entangle('priority_status').defer }" class="sm:col-span-2">
                        ...
                        <div x-show="showPlanned === 'planned'" class="sm:col-span-2 mt-4" x-cloak class="sm:col-span-2">
                        ...
                        <div x-show="showPlanned === 'routine'" class="sm:col-span-2 mt-4" x-cloak class="sm:col-span-2">
                            <label for="routine_datetime"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-200">Routine
                                Date
                                & Time</label>
                            <input type="datetime-local" wire:model="routine_datetime" id="routine_datetime"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-600 focus:border-green-600 block w-full p-2.5
                                dark:bg-gray-600 dark:border-gray-500 dark:text-white dark:placeholder-gray-400">
                    ...
                    @if ($resolved_at)
                        <div>
                                ...
                                @if ($displayImages && count($displayImages) > 0)
                                    @foreach ($displayImages as $index => $image)
                                        ...
                                        <div class="relative shrink-0" wire:sortable.item="{{ $index }}"
                                            ...
                                            <!-- Remove Image -->
                                            <button type="button"
                                                wire:click="removeResolvedImage({{ $index }})"
                                                class="absolute top-2 right-2 bg-gray-200 text-gray-500 rounded-full w-5 h-5 flex items-center justify-center text-sm font-semibold leading-none hover:bg-red-300 hover:text-red-700 transition">
                                                ×
                                            </button>
                                        </div>
                                    ...
                                    <label for="resolvedImageInput" class="cursor-pointer shrink-0"
                                        wire:loading.remove wire:target="resolvedImages">
                                        <div
                                            class="w-52 h-40 border-2 border-dashed border-gray-400 rounded-md flex items-center justify-center text-gray-400">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <!-- Spinner loading indicator -->
                            <div wire:loading wire:target="resolvedImages"
