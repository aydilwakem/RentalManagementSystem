...
<body style="margin: 0; padding: 0; background-color: #f0f0f0; font-family: Poppins, sans-serif; color: #333333;">
    ...
    <div
        ...
        <div style="padding: 20px 30px; font-size: 16px; line-height: 1.6;">
            <h3 style="font-size: 18px; color: #333333; margin-top: 0;">Hello {{ $pending->transactionUser->first_name
                }}
                {{$pending->transactionUser->last_name }},</h3>
            <p style="margin-bottom: 15px; text-align: justify;">We regret to inform you that your pencil booking at {{
                $branding->company_name}}
                has expired. Unfortunately, we were unable to confirm your reservation because another guest has
                completed the downpayment
                for the same date and time slot on the same event hall and/or rooms.
            </p>
                ...
                <table cellpadding="0" cellspacing="0" style="width: 100%; font-size: 15px;">
                    ...
                    <tr>
                        ...
                        <td style="padding: 8px 0; border-bottom: 1px dashed #e0e0e0; text-align: right;">
                            {{ ($pending->start_datetime)->format('F j, Y, g:i A') }}
                        </td>
                    </tr>
                    <tr>
            ...
            <h3 style="color: #166534; font-size: 20px; font-weight: 600; margin-top: 30px; margin-bottom: 15px;">Guest
                Details</h3>
            <p style="margin: 5px 0;"><strong>Name:</strong> {{ $pending->transactionUser->first_name }} {{
                $pending->transactionUser->last_name }}</p>
            <p style="margin: 5px 0;"><strong>Email:</strong> {{ $pending->transactionUser->email }}</p>
            <p style="margin: 5px 0;"><strong>Contact Number:</strong> {{ $pending->transactionUser->contact_number }}
            </p>
            ...
            {{-- Catering --}}
            @if(!empty($pending->dishes) && is_array($pending->dishes) && count($pending->dishes) > 0)
            <h3 style="color: #166534; font-size: 20px; font-weight: 600; margin-top: 30px; margin-bottom: 15px;">
                Catering Dishes</h3>
            <table cellpadding="10" cellspacing="0"
                style="width: 100%; border-collapse: collapse; margin: 15px auto 25px auto;">
                ...
                @foreach ($pending->dishes as $dish)
                <tr>
            ...
            <p style="font-size: 18px; font-weight: 600; margin-top: 30px; text-align: center; color: #166534;">
                <strong>We hope to welcome you soon at {{ $branding->company_name }}!</strong>
            </p>
        </div>
