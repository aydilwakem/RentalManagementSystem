    ...
    <div
        ...
        <div
            ...
            <h1 style="font-size: 28px; font-weight: 700; margin: 0; padding-top: 10px;">
                Your Event Booking Has Expired!
            </h1>
            <p style="font-size: 16px; margin: 10px 0 20px;">Unfortunately, your pencil booking could not be confirmed.
            </p>
        </div>

        {{-- Message --}}
        <div style="padding: 20px 30px; font-size: 16px; line-height: 1.6;">
            <h3 style="font-size: 18px; color: #333333; margin-top: 0;">Hello {{
                $eventTransaction->transactionUser->first_name
                }}
                {{$eventTransaction->transactionUser->last_name }},</h3>
            <p style="margin-bottom: 15px; text-align: justify;">We regret to inform you that your pencil booking at {{
                $settings->company_name}}
                has expired as the allowed payment submission period ({{ $settings->event_payment_proof_expiration_hours
                }} hours)
                has passed without confirmation.
            </p>
            <p style="margin-bottom: 15px; text-align: justify;">
                We truly appreciate your interest in holding your event with us, and we understand how disappointing
                this may be.
                Please know that we would still be happy to assist you in finding another available date or event hall
                that suits your plans.
            </p>
            <p style="margin-bottom: 25px; text-align: justify;">
                If you have any questions or concerns, please don’t hesitate to reach out — we’re
                happy to assist you.
            </p>
                ...
                <table cellpadding="0" cellspacing="0" style="width: 100%; font-size: 15px;">
                    <tr>
                        ...
                        <td
                            ...
                            {{ $eventTransaction->transaction_number }}
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td style="padding: 8px 0; border-bottom: 1px dashed #e0e0e0; text-align: right;">
                            {{ ($eventTransaction->start_datetime)->format('F j, Y, g:i A') }}
                        </td>
                    </tr>
                    <tr>
            ...
            <h3 style="color: #166534; font-size: 20px; font-weight: 600; margin-top: 30px; margin-bottom: 15px;">Guest
                Details</h3>
            <p style="margin: 5px 0;"><strong>Name:</strong> {{ $eventTransaction->transactionUser->first_name }} {{
                $eventTransaction->transactionUser->last_name }}</p>
            <p style="margin: 5px 0;"><strong>Email:</strong> {{ $eventTransaction->transactionUser->email }}</p>
            <p style="margin: 5px 0;"><strong>Contact Number:</strong> {{
                $eventTransaction->transactionUser->contact_number }}
            </p>
            ...
            {{-- Catering --}}
            @if(!empty($eventTransaction->dishes) && is_array($eventTransaction->dishes) &&
            count($eventTransaction->dishes) > 0)
            <h3 style="color: #166534; font-size: 20px; font-weight: 600; margin-top: 30px; margin-bottom: 15px;">
                Catering Dishes</h3>
            <table cellpadding="10" cellspacing="0"
                style="width: 100%; border-collapse: collapse; margin: 15px auto 25px auto;">
                ...
                @foreach ($eventTransaction->dishes as $dish)
                <tr>
            ...
            <p style="font-size: 16px;">If you have any questions or need assistance, feel
                free to reach out to us.</p>
            <p style="font-size: 18px; font-weight: 600; margin-top: 30px; text-align: center; color: #166534;">
                <strong>We hope to welcome you soon at {{ $settings->company_name }}!</strong>
            </p>
        </div>
        ...
        <div
            ...
            <div style="margin-bottom: 10px;">
                ...
                <a href="{{ $settings->facebook }}" target="_blank" style="margin: 0 8px; text-decoration: none;">
                    <img src="{{ asset('images/fb-logo.png') }}" alt="Facebook" style="width: 24px; height: 24px;">

                </a>

                <a href="{{ $settings->instagram }}" target="_blank" style="margin: 0 8px; text-decoration: none;">
            ...
            <div style="font-size: 0.9em; margin-bottom: 15px; line-height: 1.6;">
                <p style="margin: 0;">Phone: <span style="font-weight: 500;">{{ $settings->contact_number }}</span></p>
                <p style="margin: 0;">Address: {{ $settings->address }}</p>
            </div>


            <span style="font-weight: 600; padding-top: 10px; display: block;">&copy; {{ date('Y') }} {{
                $settings->company_name }}. All rights reserved.</span>
        </div>
    </div>
</body>
